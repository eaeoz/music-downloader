...
.logo i{font-size:22px;background:linear-gradient(135deg,#7c3aed,#06b6d4);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.spinner{width:36px;height:36px;border:3px solid #2a2a44;border-top-color:#7c3aed;border-radius:50%;animation:spin .8s linear infinite}
@keyframes spin{to{transform:rotate(360deg)}}
.status{font-size:13px;color:#8888aa;text-align:center}
</style></head>
<body>
<div class="logo"><i class="fas fa-music"></i>music-dl</div>
...
function showError(msg) {
  ...
  const html = loadingHTML
    ...
    .replace(
      '<div class="status" id="status">Starting server...</div>',
      `<div class="status"><span style="color:#ef4444;font-weight:500">${msg}</span><br><br><button onclick="location.reload()" style="padding:8px 20px;border-radius:8px;border:1px solid #7c3aed;background:transparent;color:#7c3aed;cursor:pointer;font-size:13px">Retry</button></div>`
    );
  mainWindow.loadURL(html).catch(() => {});
}
...
function startBackend() {
  ...
  process.env.TRACK_DL_PATH = trackDlPath;
...
function createWindow() {
  ...
  const appPath = getAppPath();
  app.setAppUserModelId('com.musicdownloader.app');
