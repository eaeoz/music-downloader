...
function drawEllipse(rx, ry, cx, cy) {
  const pts = [];
  let x = 0, y = ry;
...
function createPNG(size) {
  ...
  const s = w / 256;
  const noteHX = Math.round(96 * s), noteHY = Math.round(175 * s);
  const stemX = Math.round(122 * s);

  fillEllipse(noteHX, noteHY, Math.round(26 * s), Math.round(18 * s), setW);
  fillRect(stemX, Math.round(68 * s), stemX + Math.round(10 * s), noteHY - Math.round(16 * s), setW);

  const flagTopY = Math.round(68 * s);
  const flagBotY = Math.round(114 * s);
  const flagRightX = Math.round(172 * s);
  for (let y = flagTopY; y <= flagBotY; y++) {
    const t = (y - flagTopY) / (flagBotY - flagTopY);
    const rightX = stemX + Math.round(10 * s) + Math.round(Math.sin(t * Math.PI) * (flagRightX - stemX - 10 * s));
    for (let x = stemX + Math.round(10 * s); x <= rightX; x++)
      setW(x, y);
  }
...
console.log('Music note icons generated in data/');
